...
<?php endif; ?>
							...
							<form method="post" action="<?php echo htmlspecialchars($_SERVER['REQUEST_URI'] . $_SERVER['QUERY_STRING']);?>" id="contact_form">
								...
								<input required="required" type="text" placeholder="<?php _e('Apellido y Nombre', 'webmoderna');?>" maxlength="50" name="ct_name" value="<?php echo htmlspecialchars(@$_SESSION['ctform']['ct_name']);?>" />
								<input required="required" type="hidden" name="do" value="contact" />

								<div class="resultado"><?php echo @$_SESSION['ctform']['email_error'];?></div>
								<input required="required" type="email" name="ct_email" id="ct_email" placeholder="E-Mail" maxlength="40" value="<?php echo htmlspecialchars(@$_SESSION['ctform']['ct_email']) ?>" />

								<div class="resultado"><?php echo @$_SESSION['ctform']['telefono_error'];?></div>
								<input required="required" type="tel" id="ct_URL" name="ct_URL" placeholder="<?php _e('Teléfono', 'webmoderna');?>" maxlength="13" value="<?php echo htmlspecialchars(@$_SESSION['ctform']['ct_URL']) ?>" />

								<div class="respuesta"><?php echo @$_SESSION['ctform']['message_error'] ?></div>
								<textarea required="required" name="ct_message" id="ct_message" maxlength="1000" placeholder="<?php _e('Escribir aquí', 'webmoderna');?>..."><?php echo htmlspecialchars(@$_SESSION['ctform']['ct_message']) ?></textarea>
								...
								<div class="verificacion">
									...
									<input required="required" class="captcha__input" type="text" id="ct_captcha" name="ct_captcha"  placeholder="abc12" maxlength="8" />
								</div>
